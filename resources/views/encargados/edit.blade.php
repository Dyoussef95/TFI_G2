  ...
  <div class="container d-flex flex-column align-items-stretch flex-shrink-0 bg-dark text-white">
    ...
    <div class="scrollarea">
      <div class="col-md-7 col-lg-8">
        <h4 class="mb-3">Editar Encargado</h4>
        <a class="btn btn-success btn-sm" href="{{ url('/encargados'); }}" role="button">Volver</a>
        <form class="" action="{{ url('/encargados/'.$encargado->id); }}" method="POST">
        @csrf
        {{ method_field('PUT') }}

        <label for="firstName" class="form-label">Nombre del Encargado</label>
              <input type="text" class="form-control" id="firstName" name="nombre" placeholder="" value="{{$encargado->nombre}}" required>

              <label for="firstName" class="form-label">Apellido del Encargado</label>
              <input type="text" class="form-control" id="firstName" name="apellido" placeholder="" value="{{$encargado->apellido}}" required>

              <label for="firstName" class="form-label">Legajo del Encargado</label>
              <input type="number" class="form-control" id="firstName" name="legajo" placeholder="" value="{{$encargado->legajo}}" required>
             
          <hr class="my-4">

          <button class="w-100 btn btn-primary btn-lg" type="submit">Editar Encargado</button>
        </form>
        <br>
      </div>
