    ...
    <div class="scrollarea">
      <div class="col-md-7 col-lg-8">
        ...
        {{ method_field('PUT') }}
          ...
          <div class="row g-3">
              <div class="col-12">
                <label for="firstName" class="form-label">Nombre del Proyecto</label>
                <input type="text" class="form-control" id="firstName" name="nombre" value="{{$proyecto->nombre}}" required>
              </div>

              <div class="col-12">
              <label for="exampleFormControlTextarea1" class="form-label">Descripción</label>
              <textarea class="form-control" id="exampleFormControlTextarea1" rows="3" required name="descripcion">{{$proyecto->descripcion}}</textarea>
              </div>

              <div class="col-6">
                <label for="inicio" class="form-label">Inicio</label>
                <input type="date" class="form-control" id="firstName" name="inicio" value="{{ $proyecto->inicio }}" required>
              </div>

              <div class="col-6">
                <label for="inicio" class="form-label">Fin</label>
                <input type="date" class="form-control" id="firstName" name="fin" value="{{ $proyecto->fin }}" required>
              </div>

              <div class="col-12">
              <select class="form-select" aria-label="Default select example" name="idEncargado" required>
                <option value="" >Seleccione un encargado</option>
                @foreach($encargados as $encargado)
                <option value="{{$encargado->id}}"
                @isset($proyecto->encargado)
                @if($encargado->id==$proyecto->encargado->id)
                selected
                @endif
                @endisset
                >{{$encargado->nombreCompleto()}}</option>
                @endforeach
              </select>
              </div>
          ...
          <button class="w-100 btn btn-primary btn-lg" type="submit">Editar Proyecto</button>
        </form>
        <br>
      </div>
