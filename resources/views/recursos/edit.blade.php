  ...
  <div class="container d-flex flex-column align-items-stretch flex-shrink-0 bg-dark text-white">
    ...
    <div class="scrollarea">
      <div class="col-md-7 col-lg-8">
        ...
        <a class="btn btn-success btn-sm" href="{{ url('/recursos'); }}" role="button">Volver</a>
        <form class="" action="{{ url('/recursos/'.$recurso->id); }}" method="POST">
        @csrf
        {{ method_field('PUT') }}

              <label for="firstName" class="form-label">Nombre del Recurso</label>
              <input type="text" class="form-control" id="firstName" name="descripcion" placeholder="" value="{{$recurso->descripcion}}" required>
              <div class="invalid-feedback">
                Ingrese un nombre válido.
              </div>
          <hr class="my-4">

          <button class="w-100 btn btn-primary btn-lg" type="submit">Editar Recurso</button>
        </form>
        <br>
      </div>
