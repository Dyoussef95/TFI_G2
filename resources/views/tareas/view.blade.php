  ...
  <div class="container d-flex flex-column align-items-stretch flex-shrink-0 bg-dark text-white">
    ...
    <div class="scrollarea">
      ...
      <a class="btn btn-success btn-sm" href="{{ url('/proyectos'); }}" role="button">Volver</a>
        <br>
        <h4 class="mb-3">Descripcion: {{$tarea->descripcion}}</h4>
        <br>
        <h4 class="mb-3">Proyecto: {{$tarea->proyecto->nombre}}</h4>
        <br>
        <h4 class="mb-3">Inicio: {{$tarea->inicio()}}</h4>
        <h4 class="mb-3">Fin: {{$tarea->fin()}}</h4>
        <br>
        <h4 class="mb-3">Grado de Avance: {{$tarea->gradoAvance}}%</h4> 
        <br>
        <h4 class="mb-3">Costo Total: ${{$tarea->costoTotal()}}</h4>          
      </div>
      <br>
      <div class="col-12">
      <h4 class="mb-3">Recursos:</h4>
            ...
            @foreach($tarea->recursos as $recurso)
                ...
                <td>${{$recurso->pivot->costo}}</td>
                <td>
                  <form action="{{ url('/tareas/'.$tarea->id.'/recursos/'.$recurso->pivot->idRecurso);  }}" method="POST">
                    {{ method_field('DELETE') }}
                    {{ csrf_field() }}
            ...
            @endforeach
                <tr>
                <td colspan="3">
                <a class="w-100 btn btn-primary btn-lg" type="submit" href="{{ url('/tareas/'.$tarea->id.'/recursos/create'); }}">Agregar Recurso</a></td>
                </tr>
            </tbody>
            </table>            
        </div>
